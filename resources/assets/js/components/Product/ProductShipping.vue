...
<div class="col-edit-panel">
  <form v-on:submit.prevent="edit" method="post" enctype="multipart/form-data" class="flex">
    ...
    <label class="full-label">{{$trans.translation.shipping}}&nbsp;<span style="font-weight:400;">{{$trans.translation.shipping_ex}}</span></label>
    <input v-validate="'max:20'" :class="{'col-edit-input': true,'is-error': errors.has('shipping')}" type="text" v-model="shipping" name="shipping">
  </div>

    <table>
      <tr>
        <td style="width: 160px;"><label class="full-label">{{$trans.translation.shipping_time}}</label></td>
        <td><input  class="s-input" type="number" v-model="shipping_time" name="shipping_time"><span class="full-label" style="margin-left:5px;">{{$trans.translation.days}}</span></td>
      </tr>

      <tr>
        <td><label class="full-label">{{$trans.translation.shipping_free}}</label></td>
        <td>
      ...
      <tr v-show="shipping_free == false">
        <td><label class="full-label">{{$trans.translation.shipping_fee}}</label></td>
        <td><input class="s-input" type="number" v-model="shipping_fee" name="shipping_fee"><span class="full-label" style="margin-left:5px;">{{$trans.translation.baht}}</span></td>
      </tr>
    </table>
    <table>
      <tr>
