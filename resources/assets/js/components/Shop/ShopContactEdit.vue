...
<div>
  ...
  <div style="padding-left: 15px;" v-show="!contacts.length">
    <h4>{{$trans.translation.no_contact}}</h4></div>
...
export default {
  ...
  methods: {
    getContact() {
      this.$Progress.start();
      this.$http.get(this.$root.url + '/' + this.$route.params.shop + '/edit/contact').then((response) => {
