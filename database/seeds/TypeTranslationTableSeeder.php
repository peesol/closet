<?php

use Illuminate\Database\Seeder;

class TypeTranslationTableSeeder extends Seeder
{
    /**
     * Run the database seeds.
     *
     * @return void
     */
    public function run()
    {
        DB::table('category_translation')->insert([
                [
                  'category_type_id' => '2',
                  'language' => 'th',
                  'name' => 'เสื้อเชิ้ต'
                ],
                [
                  'category_type_id' => '3',
                  'language' => 'th',
                  'name' => 'เสื้อยืด & เสื้อกล้าม'
                ],
                [
                  'category_type_id' => '4',
                  'language' => 'th',
                  'name' => 'แจ็คเก็ต'
                ],
                [
                  'category_type_id' => '5',
                  'language' => 'th',
                  'name' => 'เสื้อกันหนาว'
                ],
                [
                  'category_type_id' => '6',
                  'language' => 'th',
                  'name' => 'สูท'
                ],
                [
                  'category_type_id' => '7',
                  'language' => 'th',
                  'name' => 'ชุดออกกำลังกาย'
                ],
                [
                  'category_type_id' => '8',
                  'language' => 'th',
                  'name' => 'กางเกงขายาว'
                ],
                [
                  'category_type_id' => '9',
                  'language' => 'th',
                  'name' => 'กางเกงขาสั้น'
                ],
                [
                  'category_type_id' => '10',
                  'language' => 'th',
                  'name' => 'กางเกงยีนส์'
                ],
                [
                  'category_type_id' => '11',
                  'language' => 'th',
                  'name' => 'ชุดชั้นใน'
                ],
                [
                  'category_type_id' => '12',
                  'language' => 'th',
                  'name' => 'ถุงเท้า'
                ],
                [
                  'category_type_id' => '13',
                  'language' => 'th',
                  'name' => 'ชุดว่ายน้ำ'
                ],
                [
                  'category_type_id' => '14',
                  'language' => 'th',
                  'name' => 'รองเท้ากีฬา'
                ],
                [
                  'category_type_id' => '15',
                  'language' => 'th',
                  'name' => 'รองเท้าผ้าใบ'
                ],
                [
                  'category_type_id' => '16',
                  'language' => 'th',
                  'name' => 'รองเท้าบูท'
                ],
                [
                  'category_type_id' => '17',
                  'language' => 'th',
                  'name' => 'รองเท้าโลฟเฟอร์ & สลิปออน'
                ],
                [
                  'category_type_id' => '18',
                  'language' => 'th',
                  'name' => 'รองเท้าอ็อกซ์ฟอร์ด'
                ],
                [
                  'category_type_id' => '19',
                  'language' => 'th',
                  'name' => 'รองเท้าแตะ'
                ],
                [
                  'category_type_id' => '20',
                  'language' => 'th',
                  'name' => 'แว่นกันแดด & แว่นตา'
                ],
                [
                  'category_type_id' => '21',
                  'language' => 'th',
                  'name' => 'หมวก'
                ],
                [
                  'category_type_id' => '22',
                  'language' => 'th',
                  'name' => 'เข็มขัด'
                ],
                [
                  'category_type_id' => '23',
                  'language' => 'th',
                  'name' => 'สร้อย แหวน ต่างหู'
                ],
                [
                  'category_type_id' => '24',
                  'language' => 'th',
                  'name' => 'นาฬิกา'
                ],
                [
                  'category_type_id' => '25',
                  'language' => 'th',
                  'name' => 'เนคไท & โบว์'
                ],
                [
                  'category_type_id' => '26',
                  'language' => 'th',
                  'name' => 'อื่นๆ'
                ],
                [
                  'category_type_id' => '27',
                  'language' => 'th',
                  'name' => 'ชุดเดรส'
                ],
                [
                  'category_type_id' => '28',
                  'language' => 'th',
                  'name' => 'เสื้อ'
                ],
                [
                  'category_type_id' => '29',
                  'language' => 'th',
                  'name' => 'เสื้อกันหนาว'
                ],
                [
                  'category_type_id' => '30',
                  'language' => 'th',
                  'name' => 'สูท & แจ็คเก็ต'
                ],
                [
                  'category_type_id' => '31',
                  'language' => 'th',
                  'name' => 'ชุดออกกำลังกาย'
                ],
                [
                  'category_type_id' => '32',
                  'language' => 'th',
                  'name' => 'กางเกงขายาว'
                ],
                [
                  'category_type_id' => '33',
                  'language' => 'th',
                  'name' => 'กางเกงขาสั้น'
                ],
                [
                  'category_type_id' => '34',
                  'language' => 'th',
                  'name' => 'กางเกงยีนส์'
                ],
                [
                  'category_type_id' => '35',
                  'language' => 'th',
                  'name' => 'กระโปรง'
                ],
                [
                  'category_type_id' => '36',
                  'language' => 'th',
                  'name' => 'เลกกิ้ง'
                ],
                [
                  'category_type_id' => '37',
                  'language' => 'th',
                  'name' => 'ชุดชั้นใน'
                ],
                [
                  'category_type_id' => '38',
                  'language' => 'th',
                  'name' => 'ชุดว่ายน้ำ'
                ],
                [
                  'category_type_id' => '39',
                  'language' => 'th',
                  'name' => 'ถุงเท้า'
                ],
                [
                  'category_type_id' => '40',
                  'language' => 'th',
                  'name' => 'รองเท้ากีฬา'
                ],
                [
                  'category_type_id' => '41',
                  'language' => 'th',
                  'name' => 'รองเท้าผ้าใบ'
                ],
                [
                  'category_type_id' => '42',
                  'language' => 'th',
                  'name' => 'รองเท้าบูท'
                ],
                [
                  'category_type_id' => '43',
                  'language' => 'th',
                  'name' => 'รองเท้าส้นเตี้ย'
                ],
                [
                  'category_type_id' => '44',
                  'language' => 'th',
                  'name' => 'รองเท้าส้นสูง'
                ],
                [
                  'category_type_id' => '45',
                  'language' => 'th',
                  'name' => 'รองเท้าโลฟเฟอร์ & สลิปออน'
                ],
                [
                  'category_type_id' => '46',
                  'language' => 'th',
                  'name' => 'รองเท้าอ็อกซ์ฟอร์ด'
                ],
                [
                  'category_type_id' => '47',
                  'language' => 'th',
                  'name' => 'รองเท้าแตะ'
                ],
                [
                  'category_type_id' => '48',
                  'language' => 'th',
                  'name' => 'แว่นกันแดด & แว่นตา'
                ],
                [
                  'category_type_id' => '49',
                  'language' => 'th',
                  'name' => 'หมวก'
                ],
                [
                  'category_type_id' => '50',
                  'language' => 'th',
                  'name' => 'เข็มขัด'
                ],
                [
                  'category_type_id' => '51',
                  'language' => 'th',
                  'name' => 'สร้อย แหวน ตุ้มหู'
                ],
                [
                  'category_type_id' => '52',
                  'language' => 'th',
                  'name' => 'นาฬิกา'
                ],
                [
                  'category_type_id' => '53',
                  'language' => 'th',
                  'name' => 'อื่นๆ'
                ],
                [
                  'category_type_id' => '54',
                  'language' => 'th',
                  'name' => 'กระเป๋าทั่วไป'
                ],
                [
                  'category_type_id' => '55',
                  'language' => 'th',
                  'name' => 'กระเป๋าสตางค์'
                ],
                [
                  'category_type_id' => '56',
                  'language' => 'th',
                  'name' => 'กระเป๋าถือ'
                ],
                [
                  'category_type_id' => '57',
                  'language' => 'th',
                  'name' => 'กระเป๋าสตางค์'
                ],
                [
                  'category_type_id' => '58',
                  'language' => 'th',
                  'name' => 'กระเป๋าอื่นๆ'
                ],
                [
                  'category_type_id' => '59',
                  'language' => 'th',
                  'name' => 'กระเป๋าเดินทาง'
                ],
                [
                  'category_type_id' => '60',
                  'language' => 'th',
                  'name' => 'อุปกรณ์เดินทาง'
                ],
                [
                  'category_type_id' => '61',
                  'language' => 'th',
                  'name' => 'แป้ง'
                ],
                [
                  'category_type_id' => '62',
                  'language' => 'th',
                  'name' => 'บลัชออน'
                ],
                [
                  'category_type_id' => '63',
                  'language' => 'th',
                  'name' => 'บีบี & ซีซี ครีม'
                ],
                [
                  'category_type_id' => '64',
                  'language' => 'th',
                  'name' => 'รองพื้น'
                ],
                [
                  'category_type_id' => '65',
                  'language' => 'th',
                  'name' => 'ไพร์มเมอร์'
                ],
                [
                  'category_type_id' => '66',
                  'language' => 'th',
                  'name' => 'บรอนเซอร์ & ไฮไลท์'
                ],
                [
                  'category_type_id' => '67',
                  'language' => 'th',
                  'name' => 'คอนซีลเลอร์'
                ],
                [
                  'category_type_id' => '68',
                  'language' => 'th',
                  'name' => 'มาสคาร่า'
                ],
                [
                  'category_type_id' => '69',
                  'language' => 'th',
                  'name' => 'อายไลเนอร์'
                ],
                [
                  'category_type_id' => '70',
                  'language' => 'th',
                  'name' => 'อายแชโดว์'
                ],
                [
                  'category_type_id' => '71',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์เกี่ยวกับขนตา'
                ],
                [
                  'category_type_id' => '72',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์เกี่ยวกับคิ้ว'
                ],
                [
                  'category_type_id' => '73',
                  'language' => 'th',
                  'name' => 'ลิปสติก'
                ],
                [
                  'category_type_id' => '74',
                  'language' => 'th',
                  'name' => 'ลิขวิดลิปสติก'
                ],
                [
                  'category_type_id' => '75',
                  'language' => 'th',
                  'name' => 'ลิปกลอส'
                ],
                [
                  'category_type_id' => '76',
                  'language' => 'th',
                  'name' => 'ดินสอเขียนริมฝีปาก'
                ],
                [
                  'category_type_id' => '77',
                  'language' => 'th',
                  'name' => 'ลิปทินท์'
                ],
                [
                  'category_type_id' => '78',
                  'language' => 'th',
                  'name' => 'ลิปพลัมพ์เพอร์'
                ],
                [
                  'category_type_id' => '79',
                  'language' => 'th',
                  'name' => 'ยาทาเล็บ'
                ],
                [
                  'category_type_id' => '80',
                  'language' => 'th',
                  'name' => 'น้ำยาล้างเล็บ'
                ],
                [
                  'category_type_id' => '81',
                  'language' => 'th',
                  'name' => 'ชุดแต่งเล็บ & อุปกรณ์เสริม'
                ],
                [
                  'category_type_id' => '82',
                  'language' => 'th',
                  'name' => 'อุปกรณ์สำหรับใบหน้า'
                ],
                [
                  'category_type_id' => '83',
                  'language' => 'th',
                  'name' => 'อุปกรณ์สำหรับดวงตา'
                ],
                [
                  'category_type_id' => '84',
                  'language' => 'th',
                  'name' => 'อุปกรณ์สำหรับเล็บ'
                ],
                [
                  'category_type_id' => '85',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์ทำความสะอาดผิวหน้า'
                ],
                [
                  'category_type_id' => '86',
                  'language' => 'th',
                  'name' => 'มาส์กหน้า & ครีมพอกหน้า'
                ],
                [
                  'category_type_id' => '87',
                  'language' => 'th',
                  'name' => 'มอยเจอร์ไรเซอร์ & ครีมบำรุงผิวหน้า'
                ],
                [
                  'category_type_id' => '88',
                  'language' => 'th',
                  'name' => 'โทนเนอร์และมิสท์'
                ],
                [
                  'category_type_id' => '89',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์ขัดและลอกหน้า'
                ],
                [
                  'category_type_id' => '90',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์ป้องกันแสงแดด'
                ],
                [
                  'category_type_id' => '91',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์บำรุงผิวกาย '
                ],
                [
                  'category_type_id' => '92',
                  'language' => 'th',
                  'name' => 'น้ำหอมผู้ชาย'
                ],
                [
                  'category_type_id' => '93',
                  'language' => 'th',
                  'name' => 'น้ำหอมผู้หญิง'
                ],
                [
                  'category_type_id' => '94',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์เปลี่ยนสีผม'
                ],
                [
                  'category_type_id' => '95',
                  'language' => 'th',
                  'name' => 'สเปรย์'
                ],
                [
                  'category_type_id' => '96',
                  'language' => 'th',
                  'name' => 'แว็กซ์'
                ],
                [
                  'category_type_id' => '97',
                  'language' => 'th',
                  'name' => 'โพเมด'
                ],
                [
                  'category_type_id' => '98',
                  'language' => 'th',
                  'name' => 'มูส'
                ],
                [
                  'category_type_id' => '99',
                  'language' => 'th',
                  'name' => 'เจล'
                ],
                [
                  'category_type_id' => '100',
                  'language' => 'th',
                  'name' => 'ทรีทเมนต์สำหรับผม'
                ],
                [
                  'category_type_id' => '101',
                  'language' => 'th',
                  'name' => 'แชมพู'
                ],
                [
                  'category_type_id' => '102',
                  'language' => 'th',
                  'name' => 'คอนดิชันเนอร์'
                ],
                [
                  'category_type_id' => '103',
                  'language' => 'th',
                  'name' => 'อุปกรณ์ดูแลเส้นผม'
                ],
                [
                  'category_type_id' => '104',
                  'language' => 'th',
                  'name' => 'วิก แฮร์พีซ คลิปต่อผม'
                ],
                [
                  'category_type_id' => '105',
                  'language' => 'th',
                  'name' => 'หวี แปรง'
                ],
                [
                  'category_type_id' => '106',
                  'language' => 'th',
                  'name' => 'สบู่ก้อน & ครีมอาบน้ำ'
                ],
                [
                  'category_type_id' => '107',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์บำรุงผิวกาย'
                ],
                [
                  'category_type_id' => '108',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์ขัดผิวกาย'
                ],
                [
                  'category_type_id' => '109',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์ดับกลิ่นกาย'
                ],
                [
                  'category_type_id' => '110',
                  'language' => 'th',
                  'name' => 'ผลิตภัณฑ์กำจัดขน'
                ],
        ]);
    }
}
